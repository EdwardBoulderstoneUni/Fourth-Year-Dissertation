...
\subsection{Netcode}
 latency graph from  {https://youtu.be/7jb0FOcImdg?t=501}
dead reckoning sends games state (sever always wins), not needed for both rollback and delay
...
\subsection{Design}
Side by side delay vs rollback demo
Used unity
Choose between dyanmic vs static delay/rollback buffers
Customise ping + variance + packet loss
Experiment With packet loss solutions
How many frames to send

Extremly simple controls to start (left/right/jump) Then consider if worth implmenting attacks
Networked state
 |                  \
\ /                  \
